...
const styles = {
    ...
    th: { textAlign: 'left', padding: '15px', borderBottom: '2px solid #eee', color: 'var(--color-text-muted)', fontWeight: '600' },
    td: { padding: '15px', borderBottom: '1px solid #f0f0f0', verticalAlign: 'middle' },
    actionBtn: { cursor: 'pointer', border: 'none', background: 'transparent', fontSize: '1.2rem', padding: '5px', margin: '0 5px' },
    ...
    modalContent: { background: '#fff', borderRadius: 'var(--radius-lg)', width: '450px', maxWidth: '90%', maxHeight: '85vh', overflowY: 'auto', boxShadow: 'var(--shadow-lg)', padding: 0 },
    modalHeader: { background: 'var(--color-primary)', color: 'white', padding: '1.5rem', borderRadius: 'var(--radius-lg) var(--radius-lg) 0 0', textAlign: 'center', margin: 0 },
...
const StaffTableSkeleton = () => (
    <div className="skeleton-agenda-container" style={{boxShadow: 'none', padding: 0}}>
         <div className="skeleton-table-header skeleton" style={{marginBottom: '0'}}></div>
        {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="skeleton-row" style={{borderBottom: '1px solid #f0f0f0'}}>
...
const StaffList = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingEmployee) {
                await api.put(`/employees/${editingEmployee._id}`, dataToSend, { headers });
                        ...
                        <table className="admin-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style={{ width: '23%', textAlign: 'center' }}>Contacto</th>
                                    ...
                                    <tr key={emp._id} style={{ opacity: emp.isActive ? 1 : 0.6 }}>
                                        <td data-label="Nombre">
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                ...
                                                <strong style={{ textTransform: 'capitalize', fontSize: '0.95rem' }}>{emp.name}</strong>
                                            </div>
                                        </td>
                                        <td data-label="Contacto">
                                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: '100%', gap: '4px' }}>
                                                <div style={{ fontWeight: '500', fontSize: '0.9rem', maxWidth: '250px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={emp.email}>
                                                    {emp.email}
                                                </div>
                                                <div style={{ fontSize: '0.8rem', color: '#718096', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                    <span className="material-symbols-rounded" style={{ fontSize: '14px' }}>phone</span>
                                                    {emp.phone}
                                                </div>
                                            </div>
                                        </td>
                                        <td data-label="Rol" style={{ textAlign: 'center' }}>
                                            <span style={{ 
                                                ...
                                                fontSize: '0.75rem', 
                                                fontWeight: '600',
                                                background: emp.role === 'admin' ? '#EBF8FF' : '#F7FAFC',
                                        ...
                                        <td data-label="Horario">
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', fontSize: '0.9rem', color: '#4A5568' }}>
                                                <span className="material-symbols-rounded" style={{ fontSize: '16px', color: '#A0AEC0' }}>schedule</span>
                                                {emp.workShift}
                                            </div>
                                        </td>
                                        ...
                                        <td data-label="Acciones">
                                            <div style={{ display: 'flex', justifyContent: 'center', gap: '8px' }}>
                                                <button onClick={() => handleOpenModal(emp)} className="admin-btn admin-btn-secondary" style={{ padding: '6px', width: '32px', height: '32px' }} title="Editar">
                                                    <span className="material-symbols-rounded" style={{ fontSize: '1.1rem', color: '#4A5568' }}>edit</span>
                                                </button>
                                                
                                                <button onClick={() => handleToggleStatus(emp)} className="admin-btn admin-btn-secondary" style={{ padding: '6px', width: '32px', height: '32px' }} title={emp.isActive ? "Desactivar" : "Activar"}>
                                                    <span className="material-symbols-rounded" style={{ fontSize: '1.1rem', color: emp.isActive ? '#D69E2E' : '#38A169' }}>
                                                        {emp.isActive ? 'toggle_on' : 'toggle_off'}
                                                    </span>
                                                </button>

                                                <button onClick={() => handleDelete(emp)} className="admin-btn admin-btn-secondary" style={{ padding: '6px', width: '32px', height: '32px', background: '#FFF5F5' }} title="Eliminar">
                                                    <span className="material-symbols-rounded" style={{ fontSize: '1.1rem', color: '#E53E3E' }}>delete</span>
                                                </button>
                                            </div>
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        <td colSpan="6" style={{ textAlign: 'center', padding: '3rem', color: '#A0AEC0' }}>
                                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                                                <span className="material-symbols-rounded" style={{ fontSize: '3rem', opacity: 0.5 }}>group_off</span>
            ...
            {showModal && (
                <div style={styles.modal}>
                    <div style={styles.modalContent}>
                        <h3 style={styles.modalHeader}>{editingEmployee ? 'Editar Empleado' : 'Nuevo Empleado'}</h3>
                            ...
                            <form onSubmit={handleSubmit}>
                                <div style={styles.formGroup}>
                                    ...
                                    <input
                                        ...
                                        minLength={3}
                                    />
                                    <small style={{ color: '#666', fontSize: '0.8rem' }}>Solo letras y espacios</small>
                                </div>
                                <div style={styles.formGroup}>
                                ...
                                <div style={styles.formGroup}>
                                    ...
                                    />
                                    <small style={{ color: '#666', fontSize: '0.8rem' }}>Mínimo 4 caracteres, solo letras, números y guión bajo</small>
                                </div>
                                <div style={styles.formGroup}>
                                    <label style={styles.label}>
                                        Contraseña {editingEmployee && <span style={{ fontWeight: 'normal', fontSize: '0.85rem' }}>(dejar vacío para no cambiar)</span>}
                                    </label>
                                    <div style={styles.passwordGroup}>
                                        <input
                                            ...
                                            minLength={6}
                                            placeholder={editingEmployee ? "••••••" : "Mínimo 6 caracteres"}
                                        />
                                        <button
                                            type="button"
                                    ...
                                    {!editingEmployee && <small style={{ color: '#666', fontSize: '0.8rem' }}>Mínimo 6 caracteres</small>}
                                </div>
                                <div style={styles.formGroup}>
                                    ...
                                    <input
                                        ...
                                        onFocus={(e) => {
                                            if (!formData.phone) setFormData({ ...formData, phone: '+54 9 ' });
                                        }}
                                        placeholder="+54 9 XXXXXXXXXX"
                                    />
                                    <small style={{ color: '#666', fontSize: '0.8rem' }}>Formato: +54 9 (cód. área sin 0) (número sin 15)</small>
                                </div>
                                <div style={styles.formGroup}>
                                    <label style={styles.label}>Rol</label>
                                    <select className="input" style={styles.select} value={formData.role} onChange={e => setFormData({ ...formData, role: e.target.value })}>
                                    ...
                                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                                        <input
                                            ...
                                            onChange={e => {
                                                ...
                                                setFormData({ ...formData, workShift: `${e.target.value} - ${end}` });
                                            }}
                                            required
                                        />
                                        <span> a </span>
                                        <input
                                            ...
                                            onChange={e => {
                                                ...
                                                setFormData({ ...formData, workShift: `${start} - ${e.target.value}` });
                                            }}
                                            required
                                        />
                                    </div>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '20px' }}>
                                    <button type="button" onClick={handleCloseModal} className="btn" style={{ background: '#ccc' }}>Cancelar</button>
